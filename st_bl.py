...
a=commands.getstatusoutput("sudo lvcreate   --size "+size+"M  --name  " + name + "  cloudvg")
print a
if a[0]==0:
	b=commands.getstatusoutput("sudo echo '<target " + name + ">\n backing-store /dev/cloudvg/"+name+" \n</target>'  >> /etc/tgt/targets.conf")
	print b
else :
...
if b[0]==0:
	...
	f.write("a=commands.getstatusoutput('iscsiadm --mode discoverydb --type sendtargets --portal 192.168.2.20 --discover') \n")
	f.write("b=commands.getstatusoutput('iscsiadm --mode node --targetname " + name + " --portal 192.168.2.20:3260 --login') \n")
	f.write("print a \n")
	f.write("print b \n")
	f.close()
